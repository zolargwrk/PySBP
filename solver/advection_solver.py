...
def advection_solver_2d(p, h, t0, tf):
    ...
    def u_bndry_diriclet(x, y, t):
        ub = np.sin(np.pi * (x-ax)*t) * np.sin(np.pi*(y-ay*t))
        return ub

    # set type of boundary: [left, right, bottom, top]
    btype = ['d', '-', 'd', '-']
    u = MeshTools2D.set_bndry(u, x, y, 0, btype, bnodes, u_bndry_diriclet)

    rhs_calculator = RHSCalculator.rhs_advection_2d
    self_time_marcher = TimeMarcher(u, t0, tf, rhs_calculator, rhs_data, u_bndry_diriclet, flux_type='Central')
    u = TimeMarcher.low_storage_rk4_2d(self_time_marcher, 0.75, x, y, ax, ay)
...
# u = advection_solver_2d(3, 0.5, 0, 5)
