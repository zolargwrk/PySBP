...
from solver.unsteady_advection import unsteady_advection_2d
from solver.unsteady_burgers import unsteady_burgers_2d
...
def save_results(h=0.8, t0=0, tf=1, cfl=1e-0, nrefine=2, domain_type='notperiodic', sbp_families=None,
                 sats_inviscid=None, sats_viscous=None, ps=None, save_results=False,
                 calc_eigvals=False, dim=2, stencil=('wide', 'narrow'), imp=('trad', 'elem'),
                 prob=('diffusion', 'advection_diffusion', 'advection', 'burgers'), n=25, showMesh=False,
                 p_map=1, curve_mesh=False, modify_saved=False, nu=0.2, steady=False):

    # setup default values based on input
    sbp_families, sats_inviscid, sats_viscous, ps, degrees, stencil, prob = input_defualt(sbp_families, sats_inviscid, sats_viscous, ps, stencil, prob, dim)

    # create a data tree using anytree module
    all_results = make_data_tree(sbp_families, sats_inviscid, sats_viscous, degrees, dim, stencil, imp, prob)
    ...
    if dim == 2:
        ...
        for f in range(0, len(sbp_families)):
            for s in range(0, len(sats_inviscid)):
                for sv in range(0, len(sats_viscous)):
                    for p in range(0, len(ps)):

                        # solve the Poisson problem and obtain data
                        if prob.lower() == 'advection':
                            soln = unsteady_advection_2d(ps[p], h, t0, tf, cfl, nrefine, sbp_families[f], sats_inviscid[s],
                                                         domain_type=domain_type, calc_eigvals=calc_eigvals,
                                                         showMesh=showMesh, p_map=p_map, curve_mesh=curve_mesh)
                        elif prob.lower() == 'burgers':
                            soln = unsteady_burgers_2d(ps[p], h, t0, tf, nu, cfl, nrefine, sbp_families[f], sats_inviscid[s],
                                                sats_viscous[sv], domain_type=domain_type, p_map=p_map, curve_mesh=curve_mesh, steady=steady)

                        # add data to the leaves of the tree
                        leaf = all_results.children[0].children[f].children[s].children[sv].children[p]
                        ...
                        if save_results:
                            ...
                            if modify_saved:
                                ...
                                sats_inviscid_saved_list = ['upwind', 'symmetric']
                                sats_viscous_saved_list = ['BR1', 'BR2', 'LDG', 'CDG', 'BO', 'CNG']
                                sbp_famliy_saved_list = ['gamma', 'omega', 'diage']
                                p_saved_list = [0, 1, 2, 3]
                                fmod = sbp_famliy_saved_list.index(sbp_families[f])
                                smod_inv = sats_inviscid_saved_list.index(sats_inviscid[s])
                                smod_vis = sats_viscous_saved_list.index(sats_viscous[sv])
                            ...
                            if calc_eigvals:
                                ...
                                if modify_saved:
                                    if prob.lower() == 'advection':
                                        with open(path + 'results_advection2D_eigvals.pickle', 'rb') as infile:
                                            # modify saved results
                                            saved_results = pickle.load(infile)
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.clear()
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.update(soln)
                                        with open(path + 'results_advection2D_eigvals.pickle', 'wb') as outfile:
                                            pickle.dump(saved_results, outfile)
                                    elif prob.lower() == 'burgers':
                                        with open(path + 'results_burgers2D_eigvals.pickle', 'rb') as infile:
                                            # modify saved results
                                            saved_results = pickle.load(infile)
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.clear()
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.update(soln)
                                        with open(path + 'results_burgers2D_eigvals.pickle', 'wb') as outfile:
                                            pickle.dump(saved_results, outfile)
                                else:
                                    if prob.lower() == 'advection':
                                        with open(path + 'results_advection2D_eigvals.pickle', 'wb') as outfile:
                                ...
                                if modify_saved:
                                    if prob.lower() == 'advection':
                                        with open(path + 'results_advection2D.pickle', 'rb') as infile:
                                            # modify saved results
                                            saved_results = pickle.load(infile)
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.clear()
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.update(soln)
                                        with open(path + 'results_advection2D.pickle', 'wb') as outfile:
                                            pickle.dump(saved_results, outfile)
                                    elif prob.lower() == 'burgers':
                                        with open(path + 'results_burgers2D.pickle', 'rb') as infile:
                                            # modify saved results
                                            saved_results = pickle.load(infile)
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.clear()
                                            saved_results.children[0].children[fmod].children[smod_inv].children[smod_vis].children[pmod].children[0].results.update(soln)
                                        with open(path + 'results_burgers2D.pickle', 'wb') as outfile:
                                            pickle.dump(saved_results, outfile)
                                else:
                                    if prob.lower() == 'advection':
                                        with open(path + 'results_advection2D.pickle', 'wb') as outfile:
        ...
        for f in range(len(sbp_families)):
            for t in range(len(stencil)):  # wide or narrow stencil
                for i in range(len(imp)):  # traditional or element type implementation
                    for s in range(len(sats_inviscid)):
                        for sv in range(len(sats_viscous)):
                            for p in range(len(ps)):
                                for r in range(len(prob)):
                                    ...
                                    if sbp_families[f] in ['LG', 'LGL']:
                                        nelem = 5
                                        app_correct = 1
                                    elif sbp_families[f] in ['CSBP', 'CSBP_Mattsson2013', 'CSBP_Mattsson2004', 'HGTL',
                                                             'HGT']:
                                        nelem = 1
                                        if ps[p] == 1:
                                            n = 10
                                    ...
                                    soln = advec_diff_1d(ps[p], 0, 1, nelem, sbp_families[f], 'upwind', sats_viscous[s], nrefine,
...
def analyze_results_2d(sbp_families=None, sats_inviscid=None, sats_viscous=None, ps=None, plot_by_family=False, plot_by_sat=False,
                       plot_by_sat_all=False,
                       ...
                       run_results=None, save_fig=False, prob='advection'):

    if prob.lower() == 'advection':
        path = 'C:\\Users\\Zelalem\\OneDrive - University of Toronto\\UTIAS\\Research\\PySBP\\visual\\advection2d_results\\'
    elif prob.lower() == 'burgers':
        path = 'C:\\Users\\Zelalem\\OneDrive - University of Toronto\\UTIAS\\Research\\PySBP\\visual\\burgers2d_results\\'
    ...
    # SAT and SBP family list in saved data
    sats_inviscid_saved_list = ['upwind', 'symmetric']
    if prob.lower() == 'burgers':
        sats_inviscid_saved_list = ['twopoint', 'splitform']
    ...
    # setup default values based on input
    sbp_families, sats_inviscid, sats_viscous, ps, degrees, _, _ = input_defualt(sbp_families, sats_inviscid, sats_viscous, ps)

    # setup plot options
    pltsetup_dict = plot_setup(sbp_families, sats_inviscid, dim)
            ...
            for f in range(len(sbp_families)):
                for s in range(0, len(sats_inviscid)):
                    for sv in range(len(sats_viscous)):
                        ...
                        res = all_results.children[0].children[fmod].children[smod_inv].children[smod_vis - 1].children[p].children[0].results
                        ...
                        label_fam_sol = 'SBP-{} $\;$ {} $\;$ $p={}$ $\;$ $r={:.2f}$'.format(
                            pltsetup.sbp_fam[sbp_families[f]], pltsetup.sat_name[sats_inviscid[s]], ps[p], conv_soln)
                        # label_fam_func = 'SBP-{} $\;$ {} $\;$ $p={}$ $\;$ $r={:.2f}$'.format(
                        #     pltsetup.sbp_fam[sbp_families[f]], pltsetup.sat_name[sats[s]], ps[p], conv_func)
                        # label_fam_sol = '{}(${:.2f}$)'.format(pltsetup.sat_name[sats_inviscid[s]], conv_soln)
            ...
            for s in range(0, len(sats)):
                for f in range(0, len(sbp_families)):
                    if run_results is None:
                        # SAT and SBP family list in saved data
                        fmod = sbp_famliy_saved_list.index(sbp_families[f])
                        smod = sats_viscous_saved_list.index(sats[s])
                    else:
                        fmod = f
                        smod = s
        ...
        for s in range(0, len(sats)):
            for p in range(pmin, pmax):
                for f in range(0, len(sbp_families)):
                    ...
                    label_sat_sol = 'SBP-{} $\;p={}$ (${:.2f}$)'.format(pltsetup.sbp_fam[sbp_families[f]], p + 1,
                                                                        conv_soln)
                    label_sat_func = 'SBP-{} $\;p={}$ (${:.2f}$)'.format(pltsetup.sbp_fam[sbp_families[f]], p + 1,
                                                                         conv_func)
                    plt.loglog(r.hs, r.errs_soln, pltsetup.markers_all[p][f], linewidth=pltsetup.lw - 2,
                               markersize=pltsetup.ms - 3,
                               label=label_sat_sol)
                    plt.xlabel(r'$h$')
                    plt.ylabel(r'solution error')
                    plt.legend(ncol=2, labelspacing=0.1, columnspacing=0.7, handletextpad=0.1)
                    ...
                    # plot functional convergence rates
                    plt.figure(4)
                    plt.loglog(r.hs, r.errs_func, pltsetup.markers_all[p][f], linewidth=pltsetup.lw - 2,
                               markersize=pltsetup.ms - 3,
                               label=label_sat_func)
                    plt.xlabel(r'$h$')
                    plt.ylabel(r'functional error')
        ...
        for p in range(pmin, pmax):
            for f in range(len(sbp_families)):
                for s in range(len(sats)):
                    ...
                    if save_fig:
                        plt.savefig(
                            path + '\\adj_conv_rates\\errs_adj_VarOper_{}_p{}.pdf'.format(sbp_families[f], p + 1),
                            format='pdf', bbox_inches='tight')
        ...
        for p in range(pmin, pmax):
            for f in range(len(sbp_families)):
                for s in range(len(sats)):
                    ...
                    r = SimpleNamespace(**res)
                    # # get results from saved tree file
                    # res = all_results.children[0].children[sbp_famliy_saved_list.index(sbp_families[f])].children[sats_viscous_saved_list.index(sats[s])].children[p].children[0].results
                    # # res = all_results.children[0].children[f].children[s].children[p].children[0].results
                    # r = SimpleNamespace(**res)
                    ...
                    if sats[s] in {'BR1', 'BR2', 'LDG', 'CDG'}:
                        plt.scatter(X, Y + s, s=120, marker=marker_spectrum[s], facecolors=marker_facecolor[s],
                ...
                if save_fig:
                    plt.savefig(
                        path + 'spectrum\\' + 'spectrum_{}_{}_p{}.pdf'.format(system_matrix, sbp_families[f], p + 1),
                        format='pdf', bbox_inches='tight')
                plt.show()
                plt.close()
            ...
            for f in range(len(sbp_families)):
                for s in range(len(sats)):
                    for refine in range(nrefine):
                        ...
                        # calculate spectral radius
                        cond_num = np.max(
                            np.abs(r.cond_nums[refine]))  # eig_vals[0]:eigenvalues with no grid refinement
                        nelem = r.nelems[refine]
        ...
        for p in range(pmin, pmax):
            for s in range(len(sats)):
                for f in range(len(sbp_families)):
                    ...
                    plt.plot(nelem_list, nnz_est_list, pltsetup.markers6[f], linewidth=2, markersize=0,
                             label=label_nnz_est)
                ...
                if save_fig:
                    ...
                    plt.savefig(path + 'sparsity\\' + 'nnz_{}_p{}.pdf'.format(sats[s], p + 1), format='pdf',
                                bbox_inches='tight')
                plt.show()
                plt.close()
    #######################################
                        ...
                        for s in range(len(sats)):
                            for t in range(len(stencil)):  # wide or narrow stencil
                                ...
                                plt.loglog(1 / r.dof, r.errs, marker_type[t], linewidth=line_type[t],
                                           markersize=markersize_type[t], fillstyle=fill_type[t],
                                           label='{}-{} {} ({:.2f})'.
                                           format(pltsetup.sbp_fam[sbp_families[f]],
...
def input_defualt(sbp_families=None, sats_inviscid=None, sats_viscous=None, ps=None, stencil=None, prob=None, dim=2):
    if dim == 2:
        if sbp_families is None:
            sbp_families = ['gamma', 'omega', 'diagE']
        if sats_inviscid is None:
            sats_inviscid = ['upwind', 'symmetric']
        if sats_viscous is None:
            # sats_viscous = ['BR1', 'BR2', 'LDG', 'CDG', 'BO', 'CNG']
            sats_viscous = ['BR2']

        # change to all to small or capital letters
        sbp_families = [x.lower() for x in sbp_families]
        sats_inviscid = [x.lower() for x in sats_inviscid]
        sats_viscous = [x.upper() for x in sats_viscous]
        ...
        if sats_inviscid is None:
            sats_inviscid = ['upwind', 'symmetric']
        if sats_viscous is None:
            sats_viscous = ['BR1', 'BR2', 'LDG', 'BO', 'CNG']
        if stencil is None:
            stencil = ['wide', 'narrow']
        if prob is None:
            prob = ['diffusion', 'advection_diffusion', 'advection', 'burgers']
    ...
    return sbp_families, sats_inviscid, sats_viscous, ps, degrees, stencil, prob
...
def plot_setup(sbp_families, sats, dim=2, stencil=None):
    ...
    if dim == 2:
        ...
        markers1 = ['-.sg', '-.sg', '-.*k', '-.*k', '-.or', '-.or', '-.<b', '-.<b', '--dm', '-dm', ':hc', '-hc', '--X',
                    '-X']
        markers2 = ['-xg', '-xg', '-+k', '-+k', '-2r', '-2r', '-1b', '-1b', '--dm', '-dm', ':hc', '-hc', '--X', '-X']
        # markers1 = ['--*b', '--xk', '--or', '--<g', '--Xm', '--dc', '--s']
        # markers2 = ['-*b', '-xk', '-or', '-<g', '-Xm', '-dc', '-s']
        markers3 = ['-.<b', '-.<b', '-.hm', '-.hm', '-.db', '-.db', '-.<c', '-.<c', '--dm', '-dm', ':hc', '-hc', '--X',
                    '-X']
        markers4 = ['-.*b', '-.xk', '-.or', '-.<g', '-.Xm', '-.dc', '-.s']
        markers5 = ['-*b', '-xk', '-or', '-<g', '-Xm', '-dc', '-s']
        markers6 = [':*b', '-.Xk', '--or', ':<g', ':Xm', ':dc', ':s']
    ...
    if 'NIPG' in sats:
        sat_name['NIPG'] = "NIPG "
    if 'upwind' in sats:
        sat_name['upwind'] = "upwind"
    if 'symmetric' in sats:
        sat_name['symmetric'] = "symmetric"
    if 'twopoint' in sats:
        sat_name['twopoint'] = "twopoint"
    if 'splitform' in sats:
        sat_name['splitform'] = "splitform"
    ...
    params = {'axes.labelsize': 28,
              ...
              'font.family': 'serif',
              'figure.figsize': [12, 6]}  # [12,6],[6,8],[12,10]
...
def make_data_tree(sbp_families, sats_inviscid, sats_viscous, degrees, dim=2, stencil=('wide', 'narrow'), imp=('trad', 'elem'),
                   prob=('diffusion', 'advection_diffusion', 'advection', 'burgers')):
    # create a data tree using anytree module
    ...
    if dim == 2:
        for f in range(len(sbp_families)):  # 2D sbp families
            ...
            for s in range(len(sats_inviscid)):  # sat types
                anytree.Node(sats_inviscid[s], parent=sbp_family.children[f])
                for sv in range(len(sats_viscous)):  # sat types
                    anytree.Node(sats_viscous[sv], parent=sbp_family.children[f].children[s])
                    for d in range(len(degrees)):  # operator degree
                        anytree.Node(degrees[d], parent=sbp_family.children[f].children[s].children[sv])
    elif dim == 1:
        for f in range(len(sbp_families)):  # operator types (Del Rey's or Mattsson's)
            ...
            for t in range(len(stencil)):  # wide or narrow stencil
                ...
                for i in range(len(imp)):  # traditional or element type implementation
                    ...
                    for s in range(len(sats_inviscid)):  # sat types
                        anytree.Node(sats_inviscid[s], parent=sbp_family.children[f].children[t].children[i])
                        for sv in range(len(sats_viscous)):  # sat types
                            anytree.Node(sats_viscous[sv], parent=sbp_family.children[f].children[t].children[i])
                            for d in range(len(degrees)):  # operator degree
                                anytree.Node(degrees[d], parent=sbp_family.children[f].children[t].children[i].children[s])
                                for r in range(len(prob)):
                                    anytree.Node(prob[r],
                                                 parent=sbp_family.children[f].children[t].children[i].children[s].children[
                                                     d])
...
def nnz_estimate(sbp_family, sat, p, nelems, dim=2):
    ...
    if sbp_family == 'omega':
        if sat == 'BR1':
            ...
            nnz_est = (np.ceil(nelems / (d + 1)) * (d ** 2 + 2) + np.floor(d * nelems / (d + 1)) * (
                        d ** 2 + 1)) * nnodes ** 2
        ...
        if sat == 'BR1':
            ...
            nnz_est = (nnodes ** 2 * nelems + (d + 1) * nnodes * nf * nelems + (
                        (d ** 2 + 1) * nelems - np.ceil(nelems / (d + 1)) * (d + 1)
...
# sat_inv = ['upwind', 'symmetric']
# sat_vis = ['BR1', 'BR2', 'LDG', 'CDG', 'BO', 'CNG']
# p = [1,2,3,4]
fam = ['gamma']
# sat_inv=['upwind']
sat_inv = ['twopoint', 'splitform']
sat_vis = ['BR2']
p = [3]
p_map = 2
...
soln = None
# advection
# soln = save_results(h=0.7, t0=0.0, tf=0.5, cfl=1e-0, nrefine=3, domain_type='periodic', sbp_families=fam,
#              sats_inviscid=sat_inv, ps=p, save_results=save_runs, calc_eigvals=calc_eigs, dim=2, prob='advection'
#              showMesh=False, p_map=p_map, curve_mesh=curve_mesh, modify_saved=modify_saved)

# analyze_results_2d(sats_inviscid=sat_inv, sbp_families=fam, ps=p, plot_by_family=plt_fam, plot_by_sat=plt_sat,  plot_by_sat_all=plt_sat_all,
#                    plot_spectrum=plt_eig, plot_spectral_radius=plt_rho, plot_sparsity=plt_sparsity,
#                    plot_adj_by_sat=plt_adj_sat, plot_adj_by_family=plt_adj_fam, tabulate_cond_num=tab_cond, prob='advection',
#                    tabulate_density=tab_density, tabulate_nnz= tab_nnz, run_results=soln, save_fig=save_figure)
# burgers
soln = save_results(h=1, t0=0.0, tf=0.01, cfl=1e-0, nrefine=4, domain_type='notperiodic', sbp_families=fam,
             sats_inviscid=sat_inv, ps=p, save_results=save_runs, calc_eigvals=calc_eigs, dim=2, prob='burgers',
             showMesh=False, p_map=p_map, curve_mesh=curve_mesh, modify_saved=modify_saved, nu=0.2, steady=True)

# analyze_results_2d(sats_inviscid=sat_inv, sbp_families=fam, ps=p, plot_by_family=plt_fam, plot_by_sat=plt_sat,  plot_by_sat_all=plt_sat_all,
#                    plot_spectrum=plt_eig, plot_spectral_radius=plt_rho, plot_sparsity=plt_sparsity,
#                    plot_adj_by_sat=plt_adj_sat, plot_adj_by_family=plt_adj_fam, tabulate_cond_num=tab_cond, prob='burgers',
#                    tabulate_density=tab_density, tabulate_nnz= tab_nnz, run_results=soln, save_fig=save_figure)
# ==================================================================================================================== #
